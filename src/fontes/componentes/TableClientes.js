...
import { faList,faEdit,faTrash,faToggleOn,faToggleOff,faPlus} from '@fortawesome/free-solid-svg-icons'
...
import ComToogle from "./ComToogle.js";
...
function TableCliente(props) {
    ...
    console.log('entrei_aqui_inicio_componente');
    ...
    const [toastrefreshpage,seToastrefreshpage] = useState(false);
    ...
    //usereffect// 
    const [iconAtivo,setIconAtivo] = useState(faToggleOn);
    const [iconInativo,setIconInativo] = useState(faToggleOff);
    ...
    const handleAtivo = (row) =>{
        console.log('handleAtivo');
    ...
    //icons//
    const faAtivo = (row) => {
       return <FontAwesomeIcon id={'row_'+row.cli_id} onClick={(e) => handleAtivo(row)} size="xl" className="pointer color_label" icon={faToggleOn} /> 
    }

    const faInativo = (row) => {
        return <FontAwesomeIcon id={'row_'+row.cli_id} onClick={(e) => handleAtivo(row)} size="xl" className="pointer color_label" icon={faToggleOff} /> 
    }
    ...
    const colunas = [
         { title: "Id",data:"cli_id", className:'dt-centro' },
         { title: "Type", data:"cli_type"},
         { title: "Name", data:"cli_name"},
         { title: "CPF/CNPJ", data:"cli_cpf_cnpj", className:'dt-centro' },
         { title: "Status", data:"cli_ativo" , className:'dt-centro' },
         { title: "Criado em", data:"created_at", className:'dt-direita' },
         { title: "Atualização", data:"updated_at", className:'dt-direita' },
         { title: "Ação", data:"action" , className:'dt-centro' },
    ];
      ...
      <>
        ...
        <div class="container-fluid px-4">
            ...
            <div class="card mb-4">
                ...
                <ButtonGrid icon={faPlus} click={NovoCliente} textoButton="Novo Cliente"></ButtonGrid>
                <div class="card-body">  
                    {loading ? (
                        <></> 
                        ) : (
                        <DataTable data={dados}  options={{ responsive: true,select: true, }} columns={colunas} className="display" slots={
                            {
                                4:(data, row) => (
                                    ...
                                    <>
                                    {row.cli_ativo == 1 ? <ComToogle acao={true} funcao={() => funcAtivo(row)}/> : <ComToogle acao={false} funcao={() => funcAtivo(row)}/>} 
                                    </>
                                ),
                                7: (data, row) => (
                                    <> 
